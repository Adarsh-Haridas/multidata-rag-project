...
class LocalStorageBackend(StorageBackend):
    ...
    def _get_doc_path(self, document_id: str) -> Path:
        """
    ...
    def save_document(self, document_id: str, file_path: Path, file_extention: str) -> None:
        ...
        destination = doc_path / f"document.{file_extention}"
    ...
    def save_chunks(self, document_id: str, file_extention: str, chunks: List[Dict]) ->None:
        ...
        chunk_file = doc_path / "chunks.json"
        with open(chunk_file, "w") as f:
            json.dump(chunks, f, indent=2)

        logger.debug(f"Saved {len(chunks)} chunks to {chunk_file}")
    ...
    def save_embeddings(self, document_id: str, file_extention: str, embeddings: np.ndarray) ->None:
        ...
        embedding_file = doc_path / "embeddings.npy"
        np.save(embedding_file, embeddings)
        logger.debug(f"Saved embeddings {embeddings.shape} to {embedding_file}")
    ...
    def load_chunks(self, document_id: str, file_extention: str) -> List[Dict]:
        ...
        """
        chunks_file = self._get_doc_path(document_id) / "chunks.json"
        ...
        logger.debug(f"Loaded {len(chunks)} from {chunks_file}")
        return chunks
    ...
    def load_embeddings(self, document_id: str, file_extention: str) -> np.ndarray:
        ...
        """
        embeddings_file = self._get_doc_path(document_id) / "embeddings.npy"
    ...
    def load_metadata(self, document_id: str, file_extention: str) ->Dict:
        ...
        """
        metadata_file = self._get_doc_path(document_id) / "metadata.json"
    ...
    def delete(self, document_id: str, file_extention: str) -> None:
        ...
        if doc_path.exists():
            ...
            logger.info(f"Deleted cache document for {document_id}")
        else:
    ...
    def get_stats(self) ->Dict:
        ...
        stats = {
            ...
            "total_size_in-mb": round(total_size / (1024*1024), 2)
        }

        logger.debug(f"Local storage stats: {stats}")
        return stats
