...
class S3StorageBackend(StorageBackend):
    ...
    def __init__(self, bucket_name: str = None):
        ...
        logger.info(f"S3 storage initialized with bucket: {self.bucket_name} ({self.region})")
    ...
    def _validate_bucket(self) -> None:
        ...
        try:
            ...
            if error_code == 404:
                raise ValueError(f"S3 bucket {self.bucket_name} does not exists")
            elif error_code == 403:
    ...
    def exists(self, document_id: str, file_extention: str) ->bool:
        ...
        for filename in required_files:
            key = self._get_s3_key(document_id, file_extention, filename)
    ...
    def save_document(self, document_id: str, file_path: Path, file_extention: str) ->None:
        ...
        Args:
            document_id: SHA-256 hash of document
            file_path: Path of the file to be
            file_extension: File extension (pdf, txt, md, etc.)
        ...
        key = self._get_s3_key(document_id, file_extention, f"document.{file_extention}")
        ...
        try:
            ...
            logger.error(f"Failed to uploaddocument to S3: {key}")
            raise
    ...
    def save_chunks(self, document_id: str, file_extention: str, chunks: List[Dict]) ->None:
        ...
        Raises:
            Exception if upload fails
        """
        key = self._get_s3_key(document_id, file_extention, "chunks.json")
    ...
    def save_embeddings(self, document_id: str, file_extention: str, embeddings: np.ndarray) -> None:
        ...
        key = self._get_s3_key(document_id, file_extention, "embeddings.npy")
        ...
        try:
            ...
            logger.error(f"Failed to save embedding to S3: {e}")
            raise
    ...
    def save_metadata(self, document_id: str, file_extention: str, metadata: Dict) -> None:
        ...
        key = self._get_s3_key(document_id, file_extention, "metadata.json")
        ...
        try:
            ...
            logger.error(f"Failed to save metadta to S3: {e}")
            raise
    ...
    def load_chunks(self, document_id: str, file_extention: str) -> List[Dict]:
        ...
        try:
            ...
            if e.response["Error"]["Code"] == 'NoSuchKey':
                raise FileNotFoundError(f"Chunk file not fiund in s3: {key}")
            raise
    ...
    def delete(self, document_id: str, file_extention: str) -> None:
        ...
        try:
            ...
            logger.info(f"Deleted all cache for document: {document_id}")
    ...
    def get_stats(self) ->Dict:
        ...
        total_objects = 0
        total_size = 0
        object_type_counts = {}     # Count documents by type (pdf: 10, txt: 5, etc.)
        ...
        try:
            ...
            for page in paginator.paginate(Bucket = self.bucket_name):
                ...
                for obj in page["Contents"]:
                    ...
                    obj_type = obj["Key"].split("/")[0] if "/" in obj["Key"] else "unknown"
            ...
            stats = {
                ...
                "objects_by_type": object_type_counts
            }
