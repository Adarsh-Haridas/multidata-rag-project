...
class VannaAgentWrapper:
    ...
    def __init__(self, openai_api_key: str, database_url: str, pinecone_api_key: Optional[str] = None):
        ...
        """
        # Initialize OpenAI LLM with gpt-4.1-mini
        ...
        logger.info(
            "Configuring SQL llm with deterministic settings"
        ...
        def determininstic_build_payload(request):
            ...
            logger.debug(f"SQL LLM  Payload prepared,(temperature: {payload['temperature']})")
            return payload
        
        # Replace the method with our wrapper
        self.llm._build_payload = determininstic_build_payload

        # Initialize PostgreSQL Runner
        self.postgress_runner = PostgresRunner(
        ...
        self.user_resolve = SimpleUserResolver()
        ...
        self.agent = Agent(
            ...
            user_resolver=self.user_resolve,
    ...
    async def _extract_sql_from_agent(self, message: str) ->str:
        ...
        async for components in self.agent.send_message(
            request_context=request_context,
            message=message
        ):
            rich_comp = components.rich_component
    ...
    async def _execute_and_extract_results(self, sql:str) ->List[Dict[str, Any]]:
        ...
        Returns:
            List of row dictionaries
        """
        logger.info(f"Executing sql directly: {sql[:150]}")
        ...
        try:
            ...
            conn_str = self.postgress_runner.connection_string
            ...
            #     logger.info(f"Resolved hostname:{hostname} to ipv4: {ipv4_address}")
            ...
            # Being commented as am gonna use session pooler instead of direct connection string inside supabase
            ...
            try:
                ...
                logger.info(f"✓ SQL executed successfully:{len(results)} rows were returned")
...
class TextToSQLService:
    ...
    def __init__(self, database_url: str | None=None, openai_api_key: str | None = None, query_cache_service=None):
        ...
        if not self.database_url:
            raise ValueError("Database url is needed for Text-SQL Feature")
        if not self.openai_api_key:
            raise ValueError("OpenAI api key url is needed for Text-SQL Feature")
...
""")
    ...
    async def generate_sql_for_approval(self, question: str) -> Dict[str, Any]:
        ...
        if self.query_cache_service and self.query_cache_service.enabled:
            ...
            cached_results = self.query_cache_service.get(cache_key, cache_type = "sql_gen")

            if cached_results and "sql" in cached_results:
                ...
                # Store in pending_queries
                self.pending_queries[query_id] = {
                    "question": question,
                    "sql": cached_results["sql"],
                    "status": "pending__approval",
                ...
                return {
                    ...
                    "question": question,
                    "sql": cached_results['sql'],
                    "explanation": cached_results.get("explanation", "This SQL will retrieve data from your database. Please review before approving."),
        ...
        try:
            ...
            if self.query_cache_service and self.query_cache_service.enabled:
                ...
                self.query_cache_service.set(cache_key, ttl, cache_value, cache_type="sql_gen")
                logger.info(f"SQL generation cache MISS - cached for '{question[:50]}...' for (TTL: {ttl})")
    ...
    async def execute_approved_query(self, query_id: str, approved: bool) -> Dict[str, Any]:
        ...
        if not approved:
            ...
            return {
                "query_id": query_id,
                "status": "rejected",
                "message": "Query execution cancelled by the user"
            }
        ...
        if is_select_query and self.query_cache_service and self.query_cache_service.enabled:
            ...
            cache_results = self.query_cache_service.get(cache_key, cache_type="sql_result")
            ...
            if cache_results and "results" in cache_results:
                ...
                return {
                    ...
                    "result_count": cache_results['result_count'],
                    "status": "executed",
                    "cache_hit": True,
                    "cached_at": cache_results.get("executed_at")
                }
        ...
        try:
            ...
            if is_select_query and self.query_cache_service and self.query_cache_service.enabled:
                ...
                self.query_cache_service.set(cache_key,ttl,cache_value, cache_type = "sql_result")
