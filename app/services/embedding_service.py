...
class EmbeddingService:
    ...
    def __init__(self, api_key: str | None= None, query_cache_service = None):
        ...
        if not self.api_key:
            raise ValueError("OpenAI api key is required(currently use mesh api if not change base_url)")
        
        self.client = AsyncOpenAI(api_key=self.api_key, base_url="https://api.meshapi.ai/v1")
        self.model = "openai/text-embedding-3-small"        # 1536 dimensions
        self.dimension = 1536
        ...
        if self.query_cache_service and self.query_cache_service.enabled:
            ...
            for i, text in enumerate(texts):
                ...
                if cached and "embedding" in cached:
                    ...
                    embeddings.append(None)     # Place Holder
