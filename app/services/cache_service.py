...
class CacheService:
    ...
    def __init__(self, storage_backend: Optional[StorageBackend] = None):
        ...
        if storage_backend is None:
            # Auto select storage backend from config
            backend_type = getattr(settings, "STORAGE_BACKEND", "local").lower()

            if backend_type == "s3":
                try:
                    self.storage = S3StorageBackend()
                    logger.info(f"Using S3 storage ({settings.S3_CACHE_BUCKET})")
    ...
    def save_chunks_and_embeddings(
        ...
        if len(chunks) != len(embeddings):
            raise ValueError(
                f"Chunk/Embedding mismatch: {len(chunks)} chunks and {len(embeddings)} embeddings"
            )
        ...
        try:
            ...
            self.storage.save_chunks(doc_id,file_extention,chunks)
            self.storage.save_embeddings(doc_id,file_extention,embeddings_array)
            self.storage.save_metadata(doc_id,file_extention,metadata)

            logger.info(f"Cached {len(chunks)} chunks for document: {doc_id} (type: {file_extention})")
            ...
            try:
                self.storage.delete(doc_id,file_extention)
            except:
                pass
            raise Exception(f"Failed to save cache: {str(e)}")


    def load_chunks_and_embeddings(self, doc_id: str, file_extention: str) ->Optional[Dict[str, Any]]:
        """
        ...
        if not self.cache_exists(doc_id,file_extention):
            return None

        try:
            # Load all files from storage backend
            chunks = self.storage.load_chunks(doc_id, file_extention)
            embeddings_array = self.storage.load_embeddings(doc_id, file_extention)
            metadata = self.storage.load_metadata(doc_id, file_extention)
            ...
            if len(chunks) != len(embeddings_list):
                logger.error(f"Cache corruption: {len(chunks)} chunks but {len(embeddings_list)} embeddings")
                return None
    ...
    def clear_cache(self, doc_id: Optional[str]=None, file_extention: Optional[str]=None) ->Dict[str,Any]:
        """
        ...
        try:
            if doc_id:
                ...
                self.storage.delete(doc_id,file_extention)
                logger.info(f"Cleared cache for doc: {doc_id}")
                return {
                    "cleared": True,
                    "message": f'Cleared cache for {doc_id}',
                    "documents_cleared": 1
                }
