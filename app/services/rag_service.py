...
class RAGService:
    ...
    def __init__(self, api_key: str | None=None, query_cache_service = None):
        ...
        if not self.api_key:
            raise ValueError("OpenAI api key is required. Set api key in '.env'file")
    ...
    async def generate_answer(self, question: str, top_k: int = 3, namespace: str = "default", include_source: bool = True) -> Dict[str,Any]:
        """
        ...
        try:
            ...
            if self.query_cache_service and self.query_cache_service.enabled:
                ...
                cache_results = self.query_cache_service.get(cache_key, cache_type = 'rag')
            ...
            embeddings,embedding_usuage = await self.embedding_service.generate_embeddings([question])
            ...
            combined_usage = {
                "embedding_tokens": embedding_usuage.get('total_tokens', 0) if embedding_usuage else 0,
                "llm_prompt_tokens": llm_usuage.get('prompt_tokens', 0) if llm_usuage else 0,
                "llm_completion_tokens": llm_usuage.get('completion_tokens', 0) if llm_usuage else 0,
                "total_tokens": (
                    (embedding_usuage.get('total_tokens', 0) if embedding_usuage else 0) + 
                    (llm_usuage.get('total_tokens', 0) if llm_usuage else 0)
                )
            }
            ...
            if self.query_cache_service and self.query_cache_service.enabled:
                ...
                self.query_cache_service.set(cache_key, ttl, result, cache_type="rag")
