...
from app.services.ingestion_service import DoclingService
...
async def health_check():
    ...
    any_service_available = any(service_status.values())
    ...
    return {
        ...
        "features_available": {
            "document_rag": service_status["rag_service"],
            "Text_to_SQL": service_status["sql_service"],
        ...
        "configuration": {
            ...
            "opik_configured": settings.OPIK_API_KEY if hasattr(settings, "OPIK_API_KEY") else False,
            "redis_cache_configured": settings.UPSTASH_REDIS_TOKEN is not None and settings.UPSTASH_REDIS_URL is not None
        },
...
async def upload_document(file: UploadFile = File(...)):
    ...
    global docling_service, embedding_service, vector_service , query_cache_service, cache_service
    ...
    try:
        ...
        if cache_service:
            try:
                ...
                logger.info(f"Document ID Computed: {doc_id[:8]}")

                # Check if cache exists for this document (pass file_extension)
                if cache_service.cache_exists(doc_id, file_extension):
                    ...
                    if cached_data:
                        ...
                        logger.warning(f"Cache loading failed, falling back to full processing")
                else:
        ...
        if chunks is None or embeddings is None:
            ...
            parsed_doc = docling_service.convert_document(str(file_path))
            raw_chunks = docling_service.chunk_document(parsed_doc)
            chunks = docling_service.extract_metadata(raw_chunks)
            ...
            # Save to cache if cache service is available
            if cache_service and doc_id:
                try:
                    # Save original document to storage
                    cache_service.save_document(doc_id,file_path, file_extension)
                    logger.info(f"saved document to storage: {doc_id[:8]}")
                    ...
                    logger.info(f"Saved cache data(chunks, embeddings & metadata): {doc_id[:8]}")

                except Exception as e:
                    # Don't fail upload if caching fails
                    logger.warning(f"Failed to save to cache (continuing anyway): {str(e)}")

        # Store in Pinecone (always, even on cache hit - in case vector DB was cleared)
        logger.info(f"Pre-Pinecone check: chunks={chunks is None}, embeddings={embeddings is None}, "
            f"chunks_len={len(chunks) if chunks is not None else 'N/A'}, "
            f"embeddings_len={len(embeddings) if embeddings is not None else 'N/A'}, "
            f"cache_hit={cache_hit}")
        
        logger.info(f"Storing {len(chunks)} vectors to Pinecone")
        ...
        # Smart cache invalidation - clear RAG cache when new document added
        ...
        if not cache_hit and query_cache_service and query_cache_service.enabled:
            try:
                ...
                if deleted > 0:
                    ...
                    logger.info("No RAG cache keys to invalidate")
        ...
        raise HTTPException(
            status_code=500,
            detail= ErrorResponse.internal_error("upload document", str(e))
        )

@app.post("/query/documents", status_code=status.HTTP_200_OK, tags=["Query"])
@track(name="query_documents", type="llm")
async def query_documents(ques: str, top_k: int = 3):
    """
    ...
    # Valiadte inputs 
    try:
        question = QueryValidator.validate_question(ques)
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail= ErrorResponse.internal_error("query_documents", e)
        )
...
async def get_stats():
    ...
    Includes query cache statistics and cost savings estimates.
    ...
    try:
        ...
        # Get query cache statistics with cost savings
        cache_stats = None
        total_cost_saved = 0
        if query_cache_service and query_cache_service.enabled:
            try:
                ...
                cache_stats = {
                    ...
                    "overall_hit rate": f"{(sum(c['hits'] for c in stats['cache_types'].values()) / max(sum(c['total_cache_requests'] for c in stats['cache_types'].values()), 1) *100):.1f}%" 
                }
        ...
        return {
            "documents": {
                "total_uploaded": documents_count,
            ...
            "configuration": {
                ...
                "cache_ttl": {
                    ...
                    "sql_result": f"{settings.CACHE_TTL_SQL_RESULT}s"
        ...
        raise HTTPException(
            status_code=500,
            detail= ErrorResponse.internal_error("get_statistics", e)
        )

@app.get("/cache/stats", status_code=status.HTTP_201_CREATED, tags=["Cache"])
...
async def clear_cache(document_id: Optional[str] = None, file_extension: Optional[str]=None):
    """
    ...
    if document_id is not None and file_extension is None:
        raise HTTPException(
            status_code= 400,
            detail= "file_extension is required when document_id is provided"
        )
    ...
    try:
        results = cache_service.clear_cache(doc_id=document_id, file_extention=file_extension)

        return {
            "status": "success" if results['cleared'] else "failed",
            **results
        }
...
async def get_query_cache_stats():
    ...
    Query-level cache statistics showing cache hit rates for:
...
@app.delete("/cache/query/clear", status_code=status.HTTP_200_OK, tags=['Query Cache'])
async def clear_query_cache(cache_type: Optional[str] = None):
    ...
    try:
        ...
        if cache_type:
            # Validate cache_type
            valid_types = ['embedding', 'rag', 'sql_gen', 'sql_result']
            if cache_type not in valid_types:
                raise HTTPException(
                    status_code=400,
                    detail= ErrorResponse.validation_error(
                        f"Invalid cache type. Must be one of: {','.join(valid_types)}",
                        field="cache_type"
                    )
                )
            ...
            patterns = ['embedding:*', 'rag:*', 'sql_gen:*', 'sql_result:*']
            total_deleted = 0
    ...
    try:
            ...
            try:
                ...
                update_current_span(
                    tags=[
                        ...
                        "approved" if auto_approve_sql else "manual_approve"
                    ],
                    metadata={
                        "question_length": len(question),
                        "route_type": route_type,
        ...
        if route_type == "SQL":
            ...
            if auto_approve_sql:
                ...
                execution_result = await sql_service.execute_approved_query(sql_result["query_id"], approved=True)
        ...
        # Route to HYBRID (both SQL and Documents)
            ...
            if not rag_service or not sql_service:
                missing_service = []
                if not rag_service:
                    missing_service.append("RAG Service")
                if not sql_service:
                    missing_service.append("SQL Service")

                raise HTTPException(
                    status_code=503,
                    detail= f"HYBRID query requires both services. Missing: {', '.join(missing_service)}"
                )
